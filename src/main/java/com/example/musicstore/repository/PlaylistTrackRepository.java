package com.example.musicstore.repository;

import org.springframework.data.jpa.repository.JpaRepository;
import com.example.musicstore.entity.Track;
import com.example.musicstore.entity.PlaylistTrack;
import org.springframework.data.jpa.repository.Modifying;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.query.Param;
import java.util.*;


public interface PlaylistTrackRepository extends JpaRepository<PlaylistTrack, PlaylistTrack.PlaylistTrackId> {

    @Query("SELECT t FROM Track t JOIN PlaylistTrack pt ON t.id = pt.trackId WHERE pt.playlistId = :playlistId")
    List<Track> findTracksByPlaylistId(@Param("playlistId") Long playlistId);

    @Query("SELECT pt FROM PlaylistTrack pt WHERE pt.playlistId = :playlistId AND pt.trackId = :trackId")
    PlaylistTrack findByPlaylistIdAndTrackId(@Param("playlistId") Long playlistId, @Param("trackId") Long trackId);

    void deleteByPlaylistId(Long playlistId);

    @Modifying
    @Query(value = "DELETE FROM PLAYLIST_TRACK WHERE playlist_id = :playlistId", nativeQuery = true)
    void deleteLegacyPlaylistTracks(@Param("playlistId") Long playlistId);

}
